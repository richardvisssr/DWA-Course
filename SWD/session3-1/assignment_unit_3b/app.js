...
app.post("/action/:player/goto/", async (req, res) => {
  try {
    ...
    const locationDescription = (game.state.player.location =
      await game.goToLocation(locationName));
    ...
    res.status(500).json({
      error:
        "Er is een fout opgetreden bij het verplaatsen naar de nieuwe locatie.",
    });
  }
});
